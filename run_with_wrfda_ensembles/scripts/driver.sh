...
while true; do
    ...
    echo "datea = $datea"
    echo "ASSIM_INT_HOURS = $ASSIM_INT_HOURS"
...
EOF
    ...
    extract_str=""  # Initialize extract_str as an empty string
    ...
    for FILE in postassim_mean.nc preassim_mean.nc postassim_sd.nc preassim_sd.nc \
        ...
        if [ -e "$FILE" ] && [ -s "$FILE" ]; then
            ...
            if [ ! $? -eq 0 ]; then
                echo "Failed moving ${RUN_DIR}/${FILE} exiting"
                touch BOMBED
                exit
            fi
        else
...
EOF
    ...
    advance_thresh=$(echo "${ADVANCE_TIME}" | cut -b4-5)
        ...
        while [ "$keep_trying" = 'true' ]; do
            ...
            while [ ! -e "${RUN_DIR}/start_member_${n}" ]; do
                ...
                if [ "$SUPER_PLATFORM" = 'slurm' ]; then
                    ...
                    if [[ "$(squeue --name="assim_advance_${n}" --noheader | wc -l)" -eq 0 ]]; then
            ...
            while true; do
                ...
                sleep 10    # This might need to be longer
