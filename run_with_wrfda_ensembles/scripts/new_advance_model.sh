...
EOF
    ...
    if [ "$SPEC_BC" -gt 0 ]; then
        ...
        if [ "$USE_WRFVAR" -eq 1 ]; then
            ...
            echo "Using WRF-Var to update LBCs, using mean boundary file"
        else
            bdyfiles=($(ls ${CENTRALDIR}/WRF/wrfbdy_d01_*_"${ensemble_member}" | grep -v mean))
            echo "Using specified LBCs, using member-specific boundary files"
        fi
        echo "${bdyfiles[@]}"
        keylist=()
        ...
        echo "No specified LBCs, using idealized WRF"
    ...
    echo ""
    echo "About running wrf"
    echo "start date = $datea"
...
EOF
    ...
    while [ "$wrfkey" -lt "$targkey" ]; do
        echo "wrfkey = $wrfkey, targkey = $targkey"
        ...
        if [ "$SPEC_BC" -gt 0 ]; then
            ...
            if [ "$USE_WRFVAR" -eq 1 ]; then
                ...
                echo "Using WRF-Var, copied wrfbdy_d01_${iday}_${isec}_mean to wrfbdy_d01"
            else
                ${COPY} "${CENTRALDIR}/WRF/wrfbdy_d01_${iday}_${isec}_${ensemble_member}" wrfbdy_d01
                echo "Using specified LBCs, copied wrfbdy_d01_${iday}_${isec}_${ensemble_member} to wrfbdy_d01"
            fi
...
EOF
            ...
            # echo ""
            # echo "Linked fg file to wrfinput_d01_${targdays}_${targsecs}_mean"
            echo "targdays = ${targdays}"
            echo "targsecs = ${targsecs}"
#             ################################
...
#                 echo "Perts added to member ${ensemble_member} second part"
#             else
...
#             fi
            ...
            # members=$(printf "%03d" $ensemble_member)

            # SRC1="/gpfs/research/chipilskigroup/stephen_asare/icbc/test/rc/2017042706/wrfinput_d01.2017042706.e${members}"
            # SRC2="/gpfs/research/chipilskigroup/stephen_asare/icbc/test/rc/2017042706/wrfbdy_d01.e${member}"
            # echo "Copying from $SRC to ${RUN_DIR}/advance_temp${n}/wrfinput_d01"

            # Define the destination directory
            # DEST1="${RUN_DIR}/advance_temp${ensemble_member}/wrfvar_output"
            # DEST2="${RUN_DIR}/advance_temp${ensemble_member}/wrfbdy_d01"
...
EOF
        ...
        while [ "$dn" -le "$num_domains" ]; do
            ...
            ${COPY} "wrfout_d0${dn}_${END_STRING}" "wrfinput_d0${dn}" ## Debugged change here.
            dn=$((dn + 1))
        done

        # ${REMOVE} wrfout* # Changed to avoid removing wrfout files
